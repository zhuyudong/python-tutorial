"""
PYTHONPATH=. mypy tests/awesome_typing.py
mypy 类型检查工具
    https://mypy-lang.org/
    pip install mypy mypy-extensions
    mypy.ini 或 setup.cfg mypy 的配置文件，命令行参数优先级高于配置文件
    mypy --help
    mypy standard_library/typing_examples.py
    mypy --strict standard_library/typing_examples.py
...
def receive_literal(x: Literal[1, 2, "str1", "str2", True]) -> Union[int, str, bool]:
    return x


# 传入 int 或 float 类型，返回 int 或 float 类型
...
# 传入 str 类型（可选）
# NOTE: Optional[str] 是否等价于 Union[str, None]？尾部的 “= None” 是否必要？
...
print(">>>line127: ", unique_numbers)  # {1, 2, 3}
...
print(">>>line136: ", categories)  # frozenset({'fruit', 'vegetable', 'meat'})
...
def speed_limit(speed: Annotated[int, "km/h"]) -> None:
    print(">>>line189: ", "Limit is", speed, "km/h")
...
print(">>>line232: ", match_result.group())  # 123
...
def display_first_element(elements: Sequence[int]) -> None:
    if elements:
        print(">>>line281: ", elements[0])
    else:
        print(">>>line283: ", "Nothing to display")
...
box1 = Box[int](123)
print(">>>line371: ", box1.get_content())  # 123
# NOTE: 忽略警告
